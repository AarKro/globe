...
- `src/scene/createGltfRoom.js` — **the active room**: loads `empty_coffeeroom.glb` (the same café export as the unused `coffeshop_room.glb`, with all seating/tables and the ceiling already removed), recenters it (floor center → origin, floor top → y=0), scales it ×5 to meters, and places `cover_chair.glb` clones at baked per-chair placements. Returns `{ room, bounds, spawn }`. The placement table was recovered by clustering the original room's merged chair geometry offline; the empty room shares the same coordinate system. The chair-strip mechanism (`CHAIR_MESH_NAMES`) is kept but empty.
